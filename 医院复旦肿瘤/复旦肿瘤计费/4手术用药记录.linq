...
void Main()
{
	var obj = new JObject();
	JToken tmp = obj;
	foreach (var property in "soap:Envelope.soap:Body.ns2:equipOperationRecordResponse.return.root.result".Split('.'))
	{
...
static JObject[] FilterData(Prescription[] data, MongoDBContext mongo)
{
	...
	var targetIds = values.SelectMany(r => new[] { r.UniqueId, r.MedicationId }).ToArray();
	var acjs = mongo.ActionJournalCollection.AsQueryable().Where(o => targetIds.Contains(o.TargetId)).ToArray();
	var osIds = values.Select(r => r.OperationScheduleId).ToArray();
	var oss = mongo.OperationScheduleCollection.AsQueryable().Where(o => osIds.Contains(o.UniqueId)).ToArray();

	var keys = values.Select(p => $"{p.UniqueId}:手术用药记录").ToArray();
	var configs = mongo.SystemConfigCollection.AsQueryable().Where(o => keys.Contains(o.Key)).ToArray();

	var outs = values.Select(p =>
	{
		p.IsSynchronized = configs.Any(x => x.Key == $"{p.UniqueId}:手术用药记录") || p.IsSynchronized;

		var acj = acjs.Where(o => o.TargetId == p.UniqueId || o.TargetId == p.MedicationId).LastOrDefault();
		var os = oss.FirstOrDefault(o => o.UniqueId == p.OperationScheduleId);
		var obj = JObject.FromObject(p);
		obj["@opertationNo"] = p.OperationScheduleId;
		obj["@operationType"] = os?.OperationType;
		obj["@operationStartTime"] = os?.ExecutionBeginTime?.ToString("yyyy-MM-dd HH:mm:ss");
		obj["@operationEndTime"] = os?.ExecutionEndTime?.ToString("yyyy-MM-dd HH:mm:ss");
		obj["@patientId"] = p.PatientId;
		obj["@patientName"] = p.Patient?.DisplayName;
		obj["@wardCode"] = p.DepartmentSourceId;
		obj["@wardName"] = p.DepartmentSource?.DisplayName;
		obj["@bedNo"] = p.Patient?.Hospitalization?.BedNo;
		obj["@hospitalNumber"] = p.Patient?.Hospitalization?.HospitalNumber;
		...
		obj["@executeWorkNo"] = acj?.PrimaryUserId;
		obj["@executeName"] = acj?.PrimaryUserName;
		obj["@remark"] = "手术用药";
